...
(async function() {
    ...
    if (!seriesContainer || !productsContainer || !scrollContainer) {
        console.error('[TransportPage] 找不到必要的容器');
        return;
    }
    ...
    try {
        ...
        // 获取 ZNTP 品牌的所有产品
        const allProducts = ProductDataLoader.getProductsByBrand('ZNTP');

        // 获取系列数据用于侧边栏
        const seriesData = ProductDataLoader.getSeriesByBrand('ZNTP');
        ...
        console.log(`[TransportPage] 成功加载 ${allProducts.length} 个产品，${seriesData.length} 个系列`);

    } catch (error) {
        console.error('[TransportPage] 加载失败:', error);
        productsContainer.innerHTML = `
            <div class="flex flex-col items-center justify-center py-24 gap-6">
    ...
    function renderSeriesList(seriesData) {
        ...
        const allOption = {
            seriesEn: 'all',
            series: '全部',
            description: 'ZNTP 品牌的全部产品线',
            thumbnail: 'assets/images/products/business-transport.webp',
            icon: 'inventory_2'
        };

        // ZNTP 品牌暂时只显示"全部"，保留算法以备将来使用
        // 如需启用分类，将下一行改为：const allSeries = seriesData.length > 0 ? [allOption, ...seriesData] : [allOption];
        ...
        const html = allSeries.map((s, index) => {
            ...
            const iconName = s.icon || 'local_shipping';
    ...
    function renderMobileSeriesSelector(seriesData) {
        ...
        let html = `
            <button class="mobile-series-btn series-active px-4 py-2 text-sm font-bold tracking-wider whitespace-nowrap
                    ...
                    data-series="all" data-name="全部" data-desc="ZNTP 品牌的全部产品线">
                全部
            </button>
        `;

        // 各系列选项（ZNTP 目前只有全部，但保留扩展能力）
    ...
    function showFilterPanel() {
        ...
        availableSubcategories = ProductDataLoader.getSubcategoriesByBrandAndSeries('ZNTP', seriesName);
